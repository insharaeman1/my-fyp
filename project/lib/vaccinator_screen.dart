...
class _VaccinatorScreenState extends State<VaccinatorScreen> {
          ...
          return AlertDialog(
            ...
            actions: [
              ...
              ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: primaryBlue),
                onPressed: () async {
                  // FIX (line 442): capture navigator before the async gap to avoid
  ...
  // FIX (line 640): signature takes only docId — no BuildContext parameter
  Future<void> _deleteRecord(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('vaccinations').doc(docId).delete();
      if (!mounted) return;
        ...
        builder: (context, childSnapshot) {
          return StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, vacSnapshot) {
              ...
              } // FIX (line 534): closing brace for the for-loop was missing
                ...
                children: [
                  ...
                  const Padding(
                    ...
                    child: Align(
                      ...
                      child: Text("Recent Activity", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textDark)),
                    ),
                  ),
                  Expanded(
                    child: ListView.builder(
  ...
  Widget _buildListItem(BuildContext context, Map<String, dynamic> data, String docId) {
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(data['childName'] ?? "Unknown", style: const TextStyle(fontWeight: FontWeight.w600)),
        subtitle: Text("${data['vaccineType']} • ${data['status']}"),
        // FIX (line 640): removed `context` argument — _deleteRecord only takes docId
        trailing: IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent), onPressed: () => _deleteRecord(docId)),
      ),
    );
  }
